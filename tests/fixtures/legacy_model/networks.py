...
class Sine(nn.Module):
    """Sinusoidal activation (SIREN, Sitzmann et al. 2020)."""
...
class ForwardPINN(nn.Module):
    ...
    def _init_weights(self):
        if self.activation == "siren":
            # Sitzmann et al. 2020 SIREN init: sine activations keep the state
            # net's autodiff time-derivative FAITHFUL (a SIREN's derivative is
            # itself a SIREN), directly attacking the biased-dz/dt ceiling that
            # limits the GELU PINN's parameter recovery.
