...
from config import BASELINE, REGIMES, DEVICE, VAR_NAMES, Y0
...
def sample_sparse(t_ref, y_ref, n_data, noise_std, seed):
    """Pick `n_data` scattered observation times and return (noisy) samples.

    Always includes t=0 so the trajectory is anchored at the initial
    condition. The remaining points are drawn uniformly at random from the
    reference grid (without replacement). Optional Gaussian noise mimics a
    real measurement; the physics residual is what fills (and denoises) the
    ...
    if noise_std > 0:
        y_obs += rng.normal(0.0, noise_std, size=y_obs.shape)
    return t_obs, y_obs
...
def train_regime(name, t_ref, y_ref, *,
                 ...
                 n_colloc=50_000,
                 n_data=40,            # sparse: a handful of observations
                 noise_std=0.0,        # measurement noise on the observations
                 adam_epochs=8000,
                 lbfgs_steps=500,
                 lr=1e-3,
                 lam_data=1.0,
                 lam_phys=1.0,
                 lam_ic=20.0,          # strong anchor on the initial condition
                 seed=42,
                 log_every=250,
                 out_dir="."):
    ...
    # --- sparse observations (instead of 3000 dense labels) -------------
    ...
    for ep in range(1, adam_epochs + 1):
        ...
        Ld  = ((net(t_d) - y_d)**2).mean()
        Lic = ((net(t_ic) - y_ic)**2).mean()
        ...
        if ep % log_every == 0 or ep == 1:
            ...
            print(f"    Adam {ep:>5}/{adam_epochs}  "
                  ...
                  f"Lp={Lp.item():.2e}  Lic={Lic.item():.2e}  "
    ...
    # observations carried alongside the solution (for plotting)
    obs = {"t": t_obs}
    for i, vn in enumerate(VAR_NAMES):
        obs[vn] = y_obs[:, i]
