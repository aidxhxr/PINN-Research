...
T = 150.0
N = 6000
TITLE = "Core WNT-HOX Regulatory Dynamics via Forward PINN (40 sparse observations)"
MODEL = "PINN"          # legend tag for the network curves

RUN_DIR = max((d for d in glob.glob(os.path.join("runs", "*"))
               if glob.glob(os.path.join(d, "*_final.pt"))), key=os.path.getmtime)
