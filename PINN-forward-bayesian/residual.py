...
def physics_rhs(t, z, p):
    """The ODE right-hand side f(t, z, theta): the 7-vector [db, dapc, ...] with
    the parameters theta KNOWN (this is the forward problem).

    Split out from physics_residual so the derivative-free TRAPEZOIDAL integral
    residual can use f(t, z) directly, without the biased autodiff dz/dt — the
    same accuracy lever that broke the PINN derivative-bias ceiling in the
    pinn-boost / bayesian-inverse work. Using net VALUES only also makes the
    Bayesian potential a SINGLE-backward function of the weights (no expensive
    double-backward through an autodiff time-derivative), which keeps HMC over
    the ~10k network weights tractable.
    """
