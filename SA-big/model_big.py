...
def modelsys_2025_k16dynamic(p, retef, wntef, CCret, CCwnt, funcpercent,
                              ...
                              *, method="BDF", rtol=1e-3, atol=1e-6,
                              t_N=12000.0, N_t=1200):
    # NOTE (SA-big): the solver settings, horizon (t_N) and number of output
    # samples (N_t) are exposed as keyword args so the sensitivity analysis can
    # trade fidelity for speed. The original reference run used
    # method="Radau", rtol=1e-6, atol=1e-9, t_N=30000, N_t=5000 (minutes per
    # solve); for screening we use BDF / 1e-3 / t_N=12000 (~35 s per solve).
