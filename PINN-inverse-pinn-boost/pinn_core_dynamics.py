"""Core WNT-HOX regulatory dynamics -- INVERSE PINN output (no scipy).

Same 4 proteins / 4 regimes / styling as the forward twin in
PINN-smaller/forward_pinn_train/pinn_core_dynamics.py, but every curve is the
INVERSE PINN's own state-net prediction z_theta(t) -- the net trained jointly
with the 36 unknown parameters (integral-residual boost run). The `ctrl`
condition is used, i.e. the baseline forcing that matches the scipy figure.

Writes two figures next to this file:
  pinn_inverse_core_dynamics.png         (2x2 panels)
  pinn_inverse_core_dynamics_single.png  (all 16 curves on one axes)
"""
import os
import glob
import numpy as np
...
T = 150.0
N = 6000
TITLE = "Core WNT-HOX Regulatory Dynamics via Inverse PINN"
MODEL = "PINN"          # legend tag for the network curves

# the new-regime integral-residual run (state nets trained with the 36 params)
RUN_DIR = os.environ.get("RUN_DIR",
                         os.path.join("runs", "20260711_203325_integral"))
COND = "ctrl"          # baseline forcing -> comparable to the scipy figure
...
def pinn_solution(name):
    ...
    net.load_state_dict(torch.load(
        os.path.join(RUN_DIR, f"{safe}_{COND}_net.pt"), map_location=DEVICE))
    net.eval()
    with torch.no_grad():
...
if __name__ == "__main__":
    print(f"Loading inverse-PINN state nets ({COND}) from {RUN_DIR} ...")
    ...
    plot_grid(sols, refs, os.path.join(here, "pinn_inverse_core_dynamics.png"))
    plot_single(sols, os.path.join(here,
                                   "pinn_inverse_core_dynamics_single.png"))
