# Converted from modelsys_2025_k16dynamic_converted.ipynb (repo path: notebooks/modelsys_2025_k16dynamic_converted.ipynb) on 2026-09-23.
...
# Run from this directory:  cd notebooks && python3 modelsys_2025_k16dynamic_converted_notebook.py

# %% [conversion shim - not notebook code]
# The notebook never imports pyplot, yet its stored outputs show four rendered
# figures: the kernel held `plt` from a cell deleted after execution. Restored
# here so the file runs top-to-bottom. No notebook cell is modified.
...
import numpy as np
import matplotlib
# [ipython-only, skipped] %matplotlib inline

# ---- real parameters from the driver script ----
p = np.array([
...
t, Sol, treatvals = modelsys_2025_k16dynamic(
    p=p, retef=0.0015, wntef=100, CCret=1000.0, CCwnt=1000.0,
    funcpercent=1.0, k8log=False, k17log=False, gamma1=1.0)

# ---- column map (0-indexed): RA 0-12, WNT 13-20, HOX 21-26, Bt 27 ----
V, Di, Db, Bp = Sol[:, 13], Sol[:, 14], Sol[:, 15], Sol[:, 16]
Da, P, Ba, X  = Sol[:, 17], Sol[:, 18], Sol[:, 19], Sol[:, 20]
H5, M, Mi     = Sol[:, 21], Sol[:, 22], Sol[:, 23]
Ca, H13, Ci   = Sol[:, 24], Sol[:, 25], Sol[:, 26]
Bt            = Sol[:, 27]
RA, Nr, CYP   = Sol[:, 3], Sol[:, 7], Sol[:, 8]

shade_start, shade_end = 3000, 25000   # WNT-on window
c = "0.1"

def shade(ax):
    ax.axvspan(shade_start, shade_end, color="0.93", zorder=0)

def panel(ax, y, title):
    ax.plot(t, y, color=c, lw=2.2)
    shade(ax)
    ax.set_title(title, fontsize=12)
    ax.set_xlabel(r"time ($\tau$)"); ax.set_ylabel("(non-dim)")
    ax.margins(x=0)

# ---- Figure 1: WNT pathway components ----
fig1, ax = plt.subplots(2, 2, figsize=(11, 8))
panel(ax[0, 0], P,  "APC ($P$)")
panel(ax[0, 1], Ba, r"$\beta$-catenin ($B_a$)")
panel(ax[1, 0], Bt, r"$\beta$-cat:TCF ($B_t$)")
panel(ax[1, 1], Bp, r"Phospho-$\beta$-cat ($B_p$)")
fig1.suptitle("WNT pathway components under transient WNT input", fontsize=15)
fig1.tight_layout(rect=[0, 0, 1, 0.96])

# ---- Figure 2: HOX pathway components ----
fig2, ax = plt.subplots(2, 3, figsize=(14, 8))
panel(ax[0, 0], H5,  "HOXA5 ($H_a$)")
panel(ax[0, 1], H13, "HOXA13 ($H_i$)")
panel(ax[0, 2], M,   "MYC ($M$)")
panel(ax[1, 0], Mi,  "MIZ1 ($M_i$)")
panel(ax[1, 1], Ca,  "MYC:MIZ1 ($C_a$)")
panel(ax[1, 2], Ci,  r"HOXA13:$\beta$-cat ($C_i$)")
fig2.suptitle("HOX components response to transient WNT activation", fontsize=15)
fig2.tight_layout(rect=[0, 0, 1, 0.96])

# ---- Figure 3: destruction complex + Dishevelled ----
fig3, ax = plt.subplots(2, 2, figsize=(11, 8))
panel(ax[0, 0], V,  "Active Dishevelled ($V$)")
panel(ax[0, 1], Da, "Active Destruction Complex ($D_a$)")
panel(ax[1, 0], Di, "Inactive Destruction Complex ($D_i$)")
panel(ax[1, 1], Db, r"Bound Destruction Complex ($D_b$)")
fig3.suptitle("Destruction-complex dynamics under transient WNT input", fontsize=15)
fig3.tight_layout(rect=[0, 0, 1, 0.96])

# ---- Figure 4: RA subsystem ----
fig4, ax = plt.subplots(1, 3, figsize=(14, 4.2))
panel(ax[0], RA,  "Retinoic Acid ($R$)")
panel(ax[1], Nr,  "RA:RAR ($R_r$)")
panel(ax[2], CYP, "CYP26A1 ($C$)")
fig4.suptitle("RA subsystem under transient WNT input", fontsize=15)
fig4.tight_layout(rect=[0, 0, 1, 0.93])

plt.show()

print("APC range", round(P.min(), 4), round(P.max(), 4))
print("Ba range ", round(Ba.min(), 4), round(Ba.max(), 4))
