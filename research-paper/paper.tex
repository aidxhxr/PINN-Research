...
\newcommand{\bcat}{$\beta$-catenin}
% math-mode spelling, for use inside align/equation
\newcommand{\mbcat}{\beta\text{-catenin}}
\newcommand{\thP}{\theta_P}

\title{Physics-Informed Neural Networks for a Reduced WNT--HOX--Retinoic Acid
Model of Colorectal Cancer: Forward Solution, Parameter Recovery, and the
...
\begin{abstract}
Colorectal cancer initiation is driven by loss of APC function and the
consequent deregulation of WNT/\bcat{} signaling, a process that retinoic acid
(RA) signaling opposes through the HOX transcription factors. We formulate a
reduced seven-state ordinary differential equation (ODE) model of the coupled
WNT--RA--HOX network, nondimensionalize it to a system with 36 kinetic
parameters, and use it as a testbed for physics-informed neural networks
(PINNs) in both the forward and the inverse direction. Four parameter regimes
--- \emph{Normal}, \emph{Early Adenoma}, \emph{Advanced Adenoma}, and
\emph{Severe APC Loss} --- reproduce the adenoma--carcinoma sequence by
increasing WNT drive $W$ while decreasing APC functionality $\thP$.

Three questions organize the work. First, \emph{can a PINN solve this system?}
The model is forced by a fast circadian RA oscillation superimposed on a slow
all-\emph{trans} retinoic acid (ATRA) treatment pulse, a two-timescale
structure that defeats a standard multilayer perceptron through spectral bias.
A multi-scale Fourier-feature time embedding removes the failure and reproduces
the stiff \texttt{Radau} reference trajectories to sub-percent relative error.
Second, \emph{which parameters matter?} A three-method global sensitivity
analysis --- local elasticity, Morris elementary effects, and Sobol variance
decomposition --- ranks parameters by influence on the concentration readouts
and on a derived stemness index, and identifies a subset whose influence is
negligible under every method. Third, and centrally, \emph{which parameters can
actually be recovered from data?} Inverse recovery saturates well below the full
parameter set, and we localize the cause. Part of the ceiling is an artifact of
the PINN estimator: replacing the biased automatic-differentiation residual
$\dot{z}-f(z)$ with a derivative-free integral (trapezoidal) residual roughly
doubles the number of parameters recovered to within 10\% relative error. The
remainder is a property of the experiment design, and only additional
\emph{information} --- perturbation conditions that excite otherwise-dark parts
of the network --- lifts it further. Underneath both sits a genuine wall: in
high-WNT regimes \bcat{} saturates and $\thP$ becomes practically
non-identifiable. We confirm this independently through the Fisher information
matrix, Raue-style profile likelihood, and Hamiltonian Monte Carlo posteriors,
and we report a cautionary calibration failure in which a na\"ively-summed
physics likelihood produced confident but wrong posteriors. For models of this
class the binding constraint is the information content of the experiment, not
...
\subsection{Why physics-informed neural networks}
...
This paper reports the deterministic and Bayesian PINN work on the reduced
WNT--RA--HOX model. It deliberately stops short of the neural-mechanistic
hybrid (universal differential equation) extensions, which are the subject of
separate work. Its contributions are:
...
Every biological assumption above appears explicitly. In
Eq.~\eqref{eq:dimB}, WNT signaling $W$ drives \bcat{} synthesis while APC
contributes to its degradation through the mass-action term $k_{PB}PB$,
encoding the inverse relationship between the two. Activation terms are written
as Hill functions where the regulation is cooperative and transcriptional
($n_B = n_M = n_H = 2$) and as Michaelis--Menten saturation where it represents
an enzymatic or titration process. In Eq.~\eqref{eq:dimP}, $\Theta_P$ is APC
...
\subsection{The reduced seven-state model}
...
5\% bar under Morris even though its Sobol index is negligible.
...
\subsection{What the sensitivity analysis does and does not tell us}
...
\subsection{Spectral bias: what happens without Fourier features}
...
\section{The inverse problem: recovering the parameters}
\label{sec:inverse}

\subsection{Formulation and scoring}
...
\subsection{Stage 1: two parameters, one condition}
...
\subsection{Stage 2: more conditions, better conditioning}
...
\subsection{A negative result: the architecture is not the bottleneck}
...
\subsection{Diagnosis: gradient starvation and a biased derivative}
...
\subsection{Lever 1: removing the derivative}
\label{sec:integral}
...
we stand behind for this lever.
...
\subsection{Lever 2: adding information of a different kind}
...
\subsection{The classical ODE-fit comparison}
\label{sec:odefit}
...
\subsection{Which parameters recover}
...
\section{Identifiability: three views of the same wall}
\label{sec:ident}
...
\subsection{One degeneracy is analytic}

The exception is a degeneracy visible by inspection. APC functionality and the
excess-degradation coefficient enter the model only through
\begin{equation}
\delta_P(\thP) = 1 + \delta_{P1}(1-\thP),
\end{equation}
...
\subsection{The well-posed contrast}
...
values, and recomputing the Fisher matrix on identical data.
...
eliminated, by reducing the parameter set.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{profile_likelihood_advanced.png}
    \caption{Profile likelihood for the Advanced Adenoma regime. Each panel
    scans one parameter while re-optimizing the other 35; the horizontal line
    is the 95\% $\chi^2$ threshold. Narrow, sharply curved profiles are
    identifiable parameters; flat or one-sided profiles indicate parameters the
    data cannot pin down.}
    \label{fig:profile}
\end{figure}

\subsection{Summary of the verdict}
...
\section{Bayesian PINNs: posteriors, and a calibration failure}
\label{sec:bayes}
...
\subsection{The likelihood, and how we got it wrong}
\label{sec:bayesfail}
...
\textbf{max $|z|$} & \textbf{CI$_{95}$ covers truth} &
\textbf{verdict reported} \\
\midrule
...
{\sigma_{\text{prior},j}},
\qquad
\text{IDENT if } \mathrm{shrink} < 0.30,\quad
\text{WEAK if } 0.30 \le \mathrm{shrink} < 0.70,\quad
\text{NON-IDENT otherwise},
\label{eq:shrink}
\end{equation}
...
\subsection{The corrected likelihood}
...
\subsection{Bayesian forward PINN}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Regime} & \textbf{predictive cov.} & \textbf{rel. predictive width} &
\textbf{epistemic cov.} & \textbf{rel. epistemic width} \\
\midrule
...
\subsection{PINN-specific findings}
...
We built a reduced seven-state model of the WNT--RA--HOX axis in colorectal
cancer, with 36 dimensionless parameters and four regimes spanning the
adenoma--carcinoma sequence, and used it to study physics-informed neural
networks in both directions.

In the forward direction, a Fourier-feature network reproduces the reference
trajectories to 1--3\% across all regimes, and does so from 40 sparse random
observations by leaning on the physics residual, at roughly $2.4\times$ the
error of a dense-label fit using $2.5\times$ more data. The multi-timescale
retinoid forcing makes the Fourier embedding mandatory: without it the network
recovers under half the treatment-pulse amplitude and none of the circadian
ripple.

A three-method sensitivity analysis places APC functionality among the most
...
own-pathway driver, and shows the reduced model to be nearly additive, in
contrast to the 27-state network it descends from, which is strongly
interacting. It also cost us a lesson: the default proportional sampling box
pushed one parameter past a sign change in the model and produced a variance
share above one, and the ranking obtained before that was caught was
plausible-looking and wrong. It also demonstrates that a-priori sensitivity does not predict
recoverability: the most sensitive parameter in the model is among the least
recoverable.

In the inverse direction, recovery saturates well below the full parameter set,
and we separate the causes. An estimator defect --- the bias in the
automatic-differentiation residual --- costs 13 parameters and is fixed by an
...
new channels; more experiments of the same kind are close to worthless.
Underneath both lies a genuine wall, confirmed by an analytic argument, the
Fisher information spectrum, profile likelihood, and Bayesian posterior
coverage: as WNT drive increases, \bcat{} saturates, $\thP$ decouples from the
observables, and the number of near-degenerate parameter pairs grows
...
We also report a calibration failure that we believe is generic. A Bayesian
PINN whose physics likelihood summed over a dense collocation grid produced
posteriors that were extremely confident and badly wrong, and a width-based
identifiability criterion certified all 36 parameters as identifiable in all
four regimes. Posterior width is not evidence of identifiability unless the
chain is mixing and the intervals are calibrated.

The through-line is that for models of this class the binding constraint is the
experiment, not the network. That conclusion was reached mostly by exhausting
the alternatives, and we have tried to report the failures with the same care as
the successes, because in this problem the failures carry most of the
information.
